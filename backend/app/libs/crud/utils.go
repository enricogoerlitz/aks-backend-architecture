...
func CheckForeignKeysExisting(
	...
	for _, foreignKeyColumn := range foreignKeyColumns {
		...
		id, err := ReflectParser.GetFieldValue(payload, foreignKeyColumn.Field)
		if err != nil {
			return err
		}

		var count int64
		if database.DBRead.Find(model, id).Count(&count); count == 0 {
...
func CheckUniqueColumns[T any](payload reflect.Value, uniqueColumns []string) error {
	...
	for _, column := range uniqueColumns {
		dbColumn := database.DBRead.NamingStrategy.ColumnName("", column)
		columnValue, err := ReflectParser.GetFieldValue(payload, column)
		if err != nil {
			return err
		}
		...
		var obj T
		var count int64
		if database.DBRead.Where(fmt.Sprintf("%s = ?", dbColumn), columnValue).Find(&obj).Count(&count); count > 0 {
